...
def loadBiData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate,BUdroprate,data_graph_file=None):
    data_path = os.path.join(cwd,'data', dataname + 'graph')
    data_graph_path = os.path.join(cwd, 'data', dataname, 'graph')
    print("loading train set", )
    traindata_list = BiGraphDataset(fold_x_train, treeDic, tddroprate=TDdroprate, budroprate=BUdroprate, data_path=data_path, data_graph_path=data_graph_path)
    print("train no:", len(traindata_list))
    print("loading test set", )
    testdata_list = BiGraphDataset(fold_x_test, treeDic, data_path=data_path, data_graph_path=data_graph_path)
    print("test no:", len(testdata_list))
    return traindata_list, testdata_list
